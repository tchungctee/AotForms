...
namespace KhoaVuxMem
{
    ...
    public class KhoaVu
    {
        ...
        // Lấy thông tin về hệ thống hiện tại, như số lượng bộ xử lý và kích thước trang bộ nhớ.
        [DllImport("kernel32.dll")]
        private static extern void GetSystemInfo(out KhoaVu.SYSTEM_INFO lpSystemInfo);

        // Mở một tiến trình và trả về một handle tới tiến trình đó.
        [DllImport("kernel32.dll")]
        public static extern IntPtr OpenProcess(uint dwDesiredAccess, bool bInheritHandle, int dwProcessId);

        // Kiểm tra xem một tiến trình có đang chạy dưới chế độ WOW64 (Windows-on-Windows 64-bit) hay không.
        [DllImport("kernel32")]
        public static extern bool IsWow64Process(IntPtr hProcess, out bool lpSystemInfo);

        // Thay đổi bảo vệ bộ nhớ của một vùng địa chỉ trong không gian bộ nhớ của một tiến trình đã mở.
        [DllImport("kernel32.dll")]
        private static extern bool VirtualProtectEx(IntPtr hProcess, UIntPtr lpAddress, IntPtr dwSize, KhoaVu.MemoryProtection flNewProtect, out KhoaVu.MemoryProtection lpflOldProtect);

        // Ghi dữ liệu vào bộ nhớ của một tiến trình đã mở.
        [DllImport("kernel32.dll")]
        private static extern bool WriteProcessMemory(IntPtr hProcess, UIntPtr lpBaseAddress, byte[] lpBuffer, UIntPtr nSize, IntPtr lpNumberOfBytesWritten);

        // Đọc dữ liệu từ bộ nhớ của một tiến trình đã mở.
        [DllImport("kernel32.dll")]
        private static extern bool ReadProcessMemory(IntPtr hProcess, UIntPtr lpBaseAddress, [Out] byte[] lpBuffer, UIntPtr nSize, IntPtr lpNumberOfBytesRead);

        // Đóng một handle đã mở.
        [DllImport("kernel32.dll")]
        public static extern int CloseHandle(IntPtr hObject);

        // Truy vấn thông tin về một vùng bộ nhớ trong không gian địa chỉ của một tiến trình đã mở (phiên bản 64-bit).
        [DllImport("kernel32.dll", EntryPoint = "VirtualQueryEx")]
        public static extern UIntPtr Native_VirtualQueryEx(IntPtr hProcess, UIntPtr lpAddress, out KhoaVu.MEMORY_BASIC_INFORMATION64 lpBuffer, UIntPtr dwLength);

        // Truy vấn thông tin về một vùng bộ nhớ trong không gian địa chỉ của một tiến trình đã mở (phiên bản 32-bit).
        [DllImport("kernel32.dll", EntryPoint = "VirtualQueryEx")]
        public static extern UIntPtr Native_VirtualQueryEx(IntPtr hProcess, UIntPtr lpAddress, out KhoaVu.MEMORY_BASIC_INFORMATION32 lpBuffer, UIntPtr dwLength);

        // Đọc một chuỗi từ tệp cấu hình (thường là tệp .ini).
        [DllImport("kernel32.dll", CharSet = CharSet.Unicode)]
        private static extern uint GetPrivateProfileString(string lpAppName, string lpKeyName, string lpDefault, StringBuilder lpReturnedString, uint nSize, string lpFileName);

        // Đọc dữ liệu từ bộ nhớ của một tiến trình đã mở vào một buffer unmanaged.
        [DllImport("kernel32.dll")]
        ...
        public string LoadCode(string name, string file)
        {
            ...
            if (flag)
            {
                uint privateProfileString = KhoaVu.GetPrivateProfileString("codes", name, "", stringBuilder, (uint)stringBuilder.Capacity, file);
            }
            else
            {
        ...
        // Dùng cho Aimbot External
        public byte[] ReadOffset(string code, long length, string file = "")
        {
            byte[] array = new byte[length];
            UIntPtr code2 = this.GetCode(code, file, 8);
            bool flag = !KhoaVu.ReadProcessMemory(this.pHandle, code2, array, (UIntPtr)(checked((ulong)length)), IntPtr.Zero);
            byte[] result;
            if (flag)
            {
                result = null;
        ...
        public Task<IEnumerable<long>> AoBScan(long start, long end, string search, bool readable, bool writable, bool executable, string file = "")
        {
            return Task.Run<IEnumerable<long>>(delegate ()
            {
                ...
                KhoaVu.SYSTEM_INFO system_INFO = default(KhoaVu.SYSTEM_INFO);
                KhoaVu.GetSystemInfo(out system_INFO);
                ...
                KhoaVu.MEMORY_BASIC_INFORMATION memory_BASIC_INFORMATION = default(KhoaVu.MEMORY_BASIC_INFORMATION);
        ...
        public void CloseProcess()
        {
            ...
            if (!flag)
            {
                KhoaVu.CloseHandle(this.pHandle);
                this.theProc = null;
            }
        }
        ...
        private unsafe long[] CompareScan(MemoryRegionResult item, byte[] aobPattern, byte[] mask)
        {
            ...
            ulong num;
            KhoaVu.ReadProcessMemory(this.pHandle, item.CurrentBaseAddress, intPtr, (UIntPtr)((ulong)item.RegionSize), out num);
        ...
        public UIntPtr VirtualQueryEx(IntPtr hProcess, UIntPtr lpAddress, out KhoaVu.MEMORY_BASIC_INFORMATION lpBuffer)
        {
            bool flag = this.Is64Bit || IntPtr.Size == 8;
            UIntPtr result;
            if (flag)
            {
                KhoaVu.MEMORY_BASIC_INFORMATION64 memory_BASIC_INFORMATION = default(KhoaVu.MEMORY_BASIC_INFORMATION64);
                UIntPtr uintPtr = KhoaVu.Native_VirtualQueryEx(hProcess, lpAddress, out memory_BASIC_INFORMATION, new UIntPtr((uint)Marshal.SizeOf(memory_BASIC_INFORMATION)));
            ...
            {
                KhoaVu.MEMORY_BASIC_INFORMATION32 memory_BASIC_INFORMATION2 = default(KhoaVu.MEMORY_BASIC_INFORMATION32);
                UIntPtr uintPtr = KhoaVu.Native_VirtualQueryEx(hProcess, lpAddress, out memory_BASIC_INFORMATION2, new UIntPtr((uint)Marshal.SizeOf(memory_BASIC_INFORMATION2)));
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (flag6)
                    {
                        ...
                        if (flag9)
                        {
                            KhoaVu.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)this.mainModule.BaseAddress + array4[0])), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                        }
                        else
                        {
                            ...
                            if (flag10)
                            {
                                ...
                                KhoaVu.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)value + array4[0])), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                            }
                            else
                            {
                                KhoaVu.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)array4[0]), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                            }
                        }
                        ...
                        for (int j = 1; j < array4.Length; j++)
                        {
                            ...
                            KhoaVu.ReadProcessMemory(this.pHandle, uintPtr, array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (flag7)
                        {
                            ...
                            if (flag10)
                            {
                                KhoaVu.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)((int)this.mainModule.BaseAddress + array4[0]))), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                            }
                            else
                            {
                                ...
                                if (flag11)
                                {
                                    ...
                                    KhoaVu.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)((int)value + array4[0]))), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                                }
                                else
                                {
                                    KhoaVu.ReadProcessMemory(this.pHandle, (UIntPtr)((ulong)((long)array4[0])), array, (UIntPtr)((ulong)((long)size)), IntPtr.Zero);
                                }
                            }
        ...
        public bool WriteMemory(string code, string type, string write, string file = "", Encoding stringEncoding = null, bool v = false)
        {
            ...
            return KhoaVu.WriteProcessMemory(this.pHandle, code2, array, (UIntPtr)((ulong)((long)num)), IntPtr.Zero);
        }
        public bool IsAdmin()
        {
            bool result;
        ...
        public bool OpenProcess(int pid)
        {
            ...
            if (flag)
            {
                Debug.WriteLine("Run As Admin pls !");
                notify("Run As Admin pls !");
            }
            bool flag2 = pid <= 0;
            bool result;
            if (flag2)
            {
                Debug.WriteLine("Open Proccess Id 0 .");
                result = false;
            }
            else
            {
                ...
                else
                {
                    try
                    {
                        ...
                        if (flag4)
                        {
                            Debug.WriteLine("Process not running or null");
                            result = false;
                        }
                        else
                        {
                            this.pHandle = KhoaVu.OpenProcess(2035711U, true, pid);
                            Process.EnterDebugMode();
                            bool flag5 = this.pHandle == IntPtr.Zero;
                            if (flag5)
                            {
                                Debug.WriteLine("OpenProcess has failed opening a handle to the target process (GetLastWin32ErrorCode: " + Marshal.GetLastWin32Error().ToString() + ")");
                            ...
                            else
                            {
                                ...
                                this.GetModules();
                                bool flag6;
                                this.Is64Bit = (Environment.Is64BitOperatingSystem && KhoaVu.IsWow64Process(this.pHandle, out flag6) && !flag6);
        ...
        public bool ChangeProtection(string code, KhoaVu.MemoryProtection newProtection, out KhoaVu.MemoryProtection oldProtection, string file = "")
        {
            UIntPtr code2 = this.GetCode(code, file, 8);
            ...
            if (flag)
            {
                oldProtection = (KhoaVu.MemoryProtection)0U;
                result = false;
            }
            else
            {
                result = KhoaVu.VirtualProtectEx(this.pHandle, code2, (IntPtr)(this.Is64Bit ? 8 : 4), newProtection, out oldProtection);
            }
            return result;
        }
    ...
    #endregion
    #region KhoaVuxMem-region-result
